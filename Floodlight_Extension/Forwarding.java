...
import net.floodlightcontroller.core.internal.IOFAtomicUpdate;
...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener {
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		...
		if (decision != null) {
			...
			if (eth.isBroadcast() || eth.isMulticast()) {
				...
				// doForwardFlow(sw, pi, cntx, false);
				doAtomicForwardFlow(sw, pi, cntx, false);
			}
		}
	...
	protected void doAtomicForwardFlow(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx, boolean requestFlowRemovedNotifn) {
		OFPort inPort = (pi.getVersion().compareTo(OFVersion.OF_12) < 0 ? pi.getInPort() : pi.getMatch().get(MatchField.IN_PORT));
		...
		if (dstDevice != null) {
			...
			Match m = reassembleMatchInfo(cntx, sw, inPort);
			U64 cookie = AppCookie.makeCookie(FORWARDING_APP_ID, 0);
			...
			if (route != null) {
				...
				List<FlowRouteInfo[]> controlLogic = new ArrayList<FlowRouteInfo[]>();
				FlowRouteInfo[] routeToInstall = new FlowRouteInfo[1];
				
				FlowRouteInfo direct_info = new FlowRouteInfo();
				direct_info.route = route;
				direct_info.match = m;
				routeToInstall[0] = direct_info;
				controlLogic.add(routeToInstall);
				
				atomicTransactionNew(controlLogic, sw, pi);
	...
	static public Match reassembleMatchInfo(FloodlightContext cntx, IOFSwitch sw, OFPort inPort){
	...
	public void init(FloodlightModuleContext context) throws FloodlightModuleException {
		...
		this.atomicService = context.getServiceImpl(IOFAtomicUpdate.class);
	...
	public class voteLock implements Runnable {
		
		List<FlowRouteInfo[]> routeToInstall;
		IOFSwitch sw;
		OFPacketIn pi;
		
		public voteLock(List<FlowRouteInfo[]> routeToInstall, IOFSwitch sw, OFPacketIn pi) {
		...
		public void run() {
			atomicService.voteLock(routeToInstall, -1, sw, pi);
		}
	}

	private void atomicTransactionNew(List<FlowRouteInfo[]> routeToInstall, IOFSwitch sw, OFPacketIn pi) {
		Thread thread = new Thread(new voteLock(routeToInstall, sw, pi));
